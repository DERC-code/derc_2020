...
def runServo1(servo1,arg1,arg1_2):
        servo1.start(0.0)
        servo1.ChangeDutyCycle(arg1*(9.5/np.pi)+7.25)
        time.sleep(0.5)
        servo1.stop()
        print("runServo1の１つ目完了")
        servo1.start(0.0)
        servo1.ChangeDutyCycle(arg1_2*(9.5/np.pi)+7.25)
        time.sleep(0.5)
        servo1.stop()
        print("runservo1の2つ目完了")

def runServo2(servo2,arg2,arg2_2):
        servo2.start(0.0)
        servo2.ChangeDutyCycle(arg2*(9.5/np.pi)+7.25)
        time.sleep(0.5)
        servo2.stop()
        print("runServo2の１つ目完了")
        servo2.start(0.0)
        servo2.ChangeDutyCycle(arg2_2*(9.5/np.pi)+7.25)
        time.sleep(0.5)
        servo2.stop()
        print("runservo2の2つ目完了")

def runServo3(servo3,arg3,arg3_2):
        i=0
        while True:
                if(i%2==0):
                        print("i%2==0")
                        servo3.start(0.0)
                        servo3.ChangeDutyCycle(4.0)
                        time.sleep(3)
                        servo3.stop()
                else:
                        print("else")
                        servo3.start(0.0)
                        servo3.ChangeDutyCycle(7.25)
                        time.sleep(3)
                        servo3.stop()
                i+=1
        servo3.start(0.0)
        ...
        servo3.stop()
        servo3.start(0.0)
        servo3.ChangeDutyCycle(arg3_2*(9.5/np.pi)+7.25)
        time.sleep(0.5)
        servo3.stop()
...
def main():
        ...
        args2= calc(100,100,30)
        print(args)
        print(args[0]*57.2958)
        print(args[1]*57.2958)
        print(args[2]*57.2958)
        #thread_1 = threading.Thread(target=runServo1(servo1,args[0],args2[0]))
        #thread_2 = threading.Thread(target=runServo2(servo2,args[1],args2[1]))
        thread_3 = threading.Thread(target=runServo3(servo3,args[2],args2[2]))
        #thread_1.start()
        #thread_2.start()
        thread_3.start()
        time.sleep(1)
