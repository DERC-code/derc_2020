...
#     time.sleep(1.5)
    ...
    motor.set_speed(-50, 50)
    time.sleep(2)
    #目標をセット34.805580, 135.778150
    target_location_x = 34.799083
    target_location_y =135.779093
    target_rad=0.0
    #中心
    calc()
    ...
    while True:
        if phase == 0:#x秒前進
            ...
            print("phase:1")
            if -0.0001<(get_gps()[0]-target_location_x) and (get_gps()[0]-target_location_x)<0.0001:# 緯度の差が0.01以内で
                if  -0.0001<(get_gps()[1]-target_location_y) and (get_gps()[1]-target_location_y)<0.0001:# かつ経度の差が0.01以内ならば
                    phase = 2 # 次のフェーズへ
                    continue
